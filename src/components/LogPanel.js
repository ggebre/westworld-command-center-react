...
import { Log } from '../services/Log'
 ...
 const LogPanel = (props) => {
  ...
  const dummyLogs = () => {
    // this.setState(pS => ({
    //   logs: [...pS.logs, this.props.message]
    // }))
    ...
    if(props.messages.length > 0){
      props.messages.forEach(message => logs.unshift(message))
    }
    ...
    return(
      <Segment className="HQComps" id="logPanel">
        ...
        <Button onClick = {props.handleActivationDeactivationAll}
          fluid 
          
          color={props.anyDeactivatedHost ? "red" : "green"}
          // {/* This isn't always going to be the same color...*/}
          content={props.anyDeactivatedHost ? "ACTIVATE ALL" : "DECOMMISSION ALL"}
